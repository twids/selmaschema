...
test('creates a family, configures schedules, joins a parent, and audits admin support', async ({ page, browser, playwright }) => {
  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'Välkommen till Selma' })).toBeVisible();
  await page.getByLabel('Familjens namn').fill('Familjen E2E');
  await page.getByRole('button', { name: 'Skapa familj' }).click();
  await expect(page.getByRole('heading', { name: 'Familjen E2E' })).toBeVisible();
  ...
  await page.getByLabel('Namn').fill('Lovschema');
  ...
  const login = await secondApi.post('/api/auth/test-login', {
    data: { email: 'second@e2e.invalid', displayName: 'E2E Parent' },
  ...
  await expect(secondPage.getByRole('heading', { name: 'Familjen E2E' })).toBeVisible();
  ...
  await page.getByText('Familjen E2E', { exact: true }).click();
