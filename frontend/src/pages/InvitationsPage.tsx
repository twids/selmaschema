...
import { familyApi, type CreatedInvitationDto, type FamilyDto, type InvitationDto } from "../api/v2";
...
export default function InvitationsPage() {
  ...
  const reload = useCallback(() => Promise.all([familyApi.invitations(familyId), familyApi.get(familyId)])
    .then(([invitations, nextFamily]) => { setItems(invitations); setFamily(nextFamily); })
    .catch(() => setError("Bara familjens ägare kan hantera inbjudningar.")), [familyId]);
  ...
  return <Stack spacing={3}>
    ...
    <Card><CardContent><Stack spacing={2}>
      ...
      <FormControl><InputLabel id="invitation-side-label">Tillhör hem</InputLabel><Select labelId="invitation-side-label" label="Tillhör hem" value={side} onChange={(e) => setSide(e.target.value as ScheduleSide | "")}><MenuItem value="">Inget hem</MenuItem><MenuItem value="A">{family?.sideALabel ?? "Första hemmet"}</MenuItem><MenuItem value="B">{family?.sideBLabel ?? "Andra hemmet"}</MenuItem></Select></FormControl>
    ...
    <Card><CardContent><Typography variant="h6" gutterBottom>Tidigare inbjudningar</Typography>{items.map((item) => <Typography key={item.id}>{item.emailHint || "Ingen e-postledtråd"} · {item.permission} · {item.side === "A" ? family?.sideALabel : item.side === "B" ? family?.sideBLabel : "inget hem"} · {item.status} · går ut {new Date(item.expiresAt).toLocaleString("sv-SE")}</Typography>)}</CardContent></Card>
  </Stack>;
}
