...
export default function PlatformAdminPage() {
  ...
  return <Stack spacing={3}><Box><Typography variant="h3" fontWeight={700}>Familjer</Typography><Typography color="text.secondary">Supportåtgärder kräver orsak och auditeras. Privat kalendertext visas aldrig här.</Typography></Box>{error && <Alert severity="error">{error}</Alert>}
    ...
    <Dialog open={selected !== null} onClose={() => setSelected(null)} fullWidth maxWidth="md"><DialogTitle>{selected?.family.name}</DialogTitle><DialogContent><Stack spacing={3} sx={{ mt: 1 }}>
      <Stack direction="row" spacing={2}><FormControl sx={{ minWidth: 200 }}><InputLabel id="admin-family-status-label">Status</InputLabel><Select labelId="admin-family-status-label" label="Status" value={selected?.family.status ?? "Active"} disabled><MenuItem value="Active">Aktiv</MenuItem><MenuItem value="Suspended">Avstängd</MenuItem></Select></FormControl><TextField fullWidth label="Obligatorisk orsak" value={reason} onChange={(e) => setReason(e.target.value)} /></Stack>
